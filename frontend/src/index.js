...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    renderProvider(payload.provider, payload.fallback_reason, payload.metadata?.usage);
...
function renderProvider(provider, fallbackReason = "", usage = null) {
  ...
  const parts = [`Origem: ${provider || "mock"}`];
  if (fallbackReason) {
    parts.push(fallbackReason);
  }
  if (usage && (usage.total_tokens || usage.input_tokens || usage.output_tokens)) {
    const total = usage.total_tokens || (Number(usage.input_tokens || 0) + Number(usage.output_tokens || 0));
    const label = usage.estimated ? "Tokens estimados" : "Tokens";
    parts.push(`${label}: ${total} total (${usage.input_tokens || 0} entrada / ${usage.output_tokens || 0} saída)`);
  }
  providerBadge.textContent = parts.join(" - ");
  ...
  const rows = produtos
    .map((item, index) => `
      <tr>
        ...
        <td>${escapeHtml(item.codigo || "Sem código")}</td>
        <td>${escapeHtml(item.descricao || item.nome || "Sem descrição")}</td>
        <td>${escapeHtml(item.ncm || "Sem NCM")}</td>
        <td>${escapeHtml(item.cst || "Sem CST")}</td>
        <td>${escapeHtml(item.cfop || "Sem CFOP")}</td>
        <td>${escapeHtml(item.quantidade ?? "Sem quantidade")}</td>
        <td>${escapeHtml(item.unidade || "Sem unidade")}</td>
...
function installmentsCard(parcelas) {
  const rows = parcelas
    .map((item) => `
      <tr>
        <td>${escapeHtml(item.numero ?? "Sem parcela")}</td>
        <td>${escapeHtml(item.descricao || "Sem documento")}</td>
  ...
  const rows = classificacoes
    .map((item) => `
      <tr>
        ...
        <td>${escapeHtml(item.justificativa || "Sem justificativa")}</td>
      </tr>
    `)
    .join("");
...
function dataRow(label, value) {
  return `
    <div class="data-row">
      ...
      <dd>${escapeHtml(hasValue(value) ? value : `Sem ${String(label || "").toLowerCase()}`)}</dd>
    </div>
  `;
}
