...
async function refreshOptions() {
  const [peopleResponse, classificationsResponse] = await Promise.all([
    fetch(apiUrl("/api/cadastros/pessoas/?all=1")),
    fetch(apiUrl("/api/cadastros/classificacoes/?all=1")),
  ]);
  const peopleData = await peopleResponse.json();
  const classificationsData = await classificationsResponse.json();
  peopleOptions.splice(0, peopleOptions.length, ...(peopleData.results || []).map((item) => ({ id: item.id, label: item.razao_social })));
  classificationOptions.splice(0, classificationOptions.length, ...(classificationsData.results || []).map((item) => ({ id: item.id, label: `${item.tipo_label} - ${item.descricao}`, tipo: item.tipo })));
}
...
async function loadRows() {
  ...
  const response = await fetch(apiUrl(`/api/cadastros/${state.resource}/?${params.toString()}`));
  const data = await response.json();
  if (response.status === 401 || data.redirect_to) {
    redirectToGeminiGate(data);
    return;
...
async function fetchRecord(id) {
  const response = await fetch(apiUrl(`/api/cadastros/${state.resource}/${id}/`));
  const data = await response.json();
  if (response.status === 401 || data.redirect_to) {
    redirectToGeminiGate(data);
    return {};
